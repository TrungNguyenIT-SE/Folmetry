...
import { startTransition, useId, useRef, ViewTransition, type KeyboardEvent, type ReactNode } from "react";
...
export function Tabs({ items, activeId, onChange, label }: Readonly<{ items: readonly TabItem[]; activeId: string; onChange: (id: string) => void; label: string }>) {
  ...
  const transitionName = `tabs-${useId().replaceAll(":", "")}`;
  const select = (id: string): void => startTransition(() => onChange(id));
  ...
  return (
    <div className="tabs">
      <div role="tablist" aria-label={label}>
        {items.map((item, index) => <button aria-controls={`${item.id}-panel`} aria-selected={item.id === active?.id} id={`${item.id}-tab`} key={item.id} onClick={() => select(item.id)} onKeyDown={(event) => move(event, index)} ref={(node) => { if (node === null) buttons.current.delete(item.id); else buttons.current.set(item.id, node); }} role="tab" tabIndex={item.id === active?.id ? 0 : -1} type="button">{item.label}</button>)}
      </div>
      {active === undefined ? null : (
        <ViewTransition default="none" enter="atlas-tab" key={active.id} name={transitionName} share="atlas-tab">
          <div aria-labelledby={`${active.id}-tab`} id={`${active.id}-panel`} role="tabpanel" tabIndex={0}>{active.content}</div>
        </ViewTransition>
      )}
    </div>
  );
