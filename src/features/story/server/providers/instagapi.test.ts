...
describe("InstagapiProvider", () => {
  it("normalizes documented Story and Highlight shapes without exposing credit data", async () => {
    const transport = vi.fn<typeof fetch>()
      ...
      .mockResolvedValueOnce(Response.json([{ id: "highlight:1", title: "Summer", items: [{ id: "h1", media_type: 1, thumbnail_url: "https://cdn.example/h.jpg" }] }]));
    ...
    await expect(provider.getHighlightItems("name", "highlight:1", new AbortController().signal)).resolves.toMatchObject({ items: [expect.objectContaining({ id: "h1" })] });
