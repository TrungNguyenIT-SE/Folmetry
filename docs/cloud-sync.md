...
Raw Facebook files follow the same privacy boundary as Instagram. The adapter recognizes the current `connections/friends` and `connections/followers` JSON layout, stores Friends separately from Followers and Following, and supports older synthetic wrapper variants. Facebook display names are not durable IDs. Friend rows with the same normalized display name are preserved instead of merged; connection timestamps distinguish them when available, but the export still does not provide a stable identifier that can prove identity.
